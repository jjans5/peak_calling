#!/bin/bash

# USAGE:
# bash liftover_fragments.sh --i input.tsv.gz --c chain.over.chain.gz --o output.tsv.gz [--n 100000] [--tmp tmp_dir] [--ncpu 8]

set -euo pipefail  # Exit on error, undefined variables, and pipe failures

# === Parse arguments ===
add_chr="false"  # default: don't add 'chr'

while [[ $# -gt 0 ]]; do
  case "$1" in
    --i) input_fragments="$2"; shift 2 ;;
    --c) chain_file="$2"; shift 2 ;;
    --o) output_file="$2"; shift 2 ;;
    --n) num_lines="$2"; shift 2 ;;
    --tmp) tmpdir="$2"; shift 2 ;;
    --ncpu) ncpu="$2"; shift 2 ;;
    --add-chr) add_chr="true"; shift 1 ;;
    *) echo "❌ Unknown argument: $1"; exit 1 ;;
  esac
done

# === Set defaults ===
num_lines="${num_lines:-all}"
ncpu="${ncpu:-1}"

# Record start time
start_time=$(date +%s)
echo "⏱️  Started at: $(date)"

# === Validate required arguments ===
if [[ -z "${input_fragments:-}" || -z "${chain_file:-}" || -z "${output_file:-}" ]]; then
  echo "❌ ERROR: --i, --c, and --o are required"
  exit 1
fi

# === Validate all required commands ===
required_commands=("awk" "cat" "cut" "date" "grep" "gzip" "head" "mkdir" "mktemp" "paste" "rm" "seq" "split" "tee" "touch" "tr" "wc" "zcat")
missing_commands=()

for cmd in "${required_commands[@]}"; do
  if ! command -v "$cmd" &>/dev/null; then
    missing_commands+=("$cmd")
  fi
done

if [[ ${#missing_commands[@]} -gt 0 ]]; then
  echo "❌ ERROR: Required command(s) not found: ${missing_commands[*]}" >&2
  echo "   Please ensure all standard Unix utilities are installed." >&2
  exit 1
fi

# Check liftOver separately with specific instructions
if ! command -v liftOver &>/dev/null; then
  echo "❌ ERROR: liftOver not found. Load with: module load ucsc" >&2
  exit 1
fi

echo "✅ All required commands available"

# === Validate input files ===
if [[ ! -s "$input_fragments" ]]; then
  echo "❌ ERROR: Input fragments file not found or empty: $input_fragments" >&2
  exit 1
fi

if [[ ! -s "$chain_file" ]]; then
  echo "❌ ERROR: Chain file not found or empty: $chain_file" >&2
  exit 1
fi

# === Output paths ===
out_base="${output_file%.tsv.gz}"
unmapped_output="${out_base}.unmapped.tsv.gz"

# === Temp dir setup ===
if [[ -z "${tmpdir:-}" ]]; then
  # Use TMPDIR if set (often points to fast local storage), otherwise use system temp
  tmpdir=$(mktemp -d -t liftover_tmp_XXXXXXXX)
  trap 'echo "🧹 Cleaning up temp dir..."; rm -rf "$tmpdir"' EXIT
  auto_cleanup=true
else
  mkdir -p "$tmpdir"
  echo "⚠️  Using user-specified temp directory: $tmpdir"
  auto_cleanup=false
fi

echo "📁 Temp directory: $tmpdir"

# === File names ===
frag_file="${tmpdir}/subset.tsv"
bed_file="${tmpdir}/coords.bed"
lifted_all="${tmpdir}/lifted_all.bed"
unmapped_bed="${tmpdir}/unmapped_all.bed"
filtered_file="${tmpdir}/filtered.tsv"

echo "📂 Input: $input_fragments"
echo "🔗 Chain: $chain_file"
echo "📤 Output: $output_file"
echo "🔢 Lines: $num_lines"
echo "⚙️  Cores: $ncpu"
echo "🔁 Add chr prefix: $add_chr"

# === Extract fragments and create BED file in one pass ===
echo "📦 Processing input file..."
if [[ "$num_lines" == "all" ]]; then
  echo "   Using all lines from file"
  # Process in one pass: extract fragments and create BED simultaneously
  if [[ "$add_chr" == "true" ]]; then
    zcat "$input_fragments" | tee "$frag_file" | \
      awk 'BEGIN{OFS="\t"} {print "chr"$1, $2, $3}' > "$bed_file"
  else
    zcat "$input_fragments" | tee "$frag_file" | \
      cut -f1-3 > "$bed_file"
  fi
else
  echo "   Extracting top $num_lines lines"
  if [[ "$add_chr" == "true" ]]; then
    zcat "$input_fragments" | head -n "$num_lines" | \
      tee "$frag_file" | \
      awk 'BEGIN{OFS="\t"} {print "chr"$1, $2, $3}' > "$bed_file"
  else
    zcat "$input_fragments" | head -n "$num_lines" | \
      tee "$frag_file" | \
      cut -f1-3 > "$bed_file"
  fi
fi

# Count lines
line_count=$(wc -l < "$frag_file" | tr -d '[:space:]')
if ! [[ "$line_count" =~ ^[0-9]+$ ]] || [[ "$line_count" -eq 0 ]]; then
  echo "❌ ERROR: Invalid or empty fragment file" >&2
  exit 1
fi
echo "✅ Processing $line_count fragments"

# === Split into chunks ===
echo "📤 Splitting into $ncpu chunks..."
split -n l/$ncpu --numeric-suffixes=1 "$bed_file" "$tmpdir/bed_chunk_"
split -n l/$ncpu --numeric-suffixes=1 "$frag_file" "$tmpdir/frag_chunk_"

# === Run liftOver in parallel with error checking ===
echo "🚀 Running liftOver on $ncpu cores..."
liftover_start=$(date +%s)

# Calculate total input size for progress tracking
total_input_size=0
for i in $(seq -f "%02g" 1 $ncpu); do
  if [[ -f "$tmpdir/bed_chunk_$i" ]]; then
    size=$(stat -f%z "$tmpdir/bed_chunk_$i" 2>/dev/null || stat -c%s "$tmpdir/bed_chunk_$i" 2>/dev/null || echo 0)
    total_input_size=$((total_input_size + size))
  fi
done

# Start progress monitor in background
(
  last_processed=0
  while true; do
    sleep 5
    
    # Calculate total processed size (lifted + unmapped)
    total_processed=0
    completed_chunks=0
    
    for i in $(seq -f "%02g" 1 $ncpu); do
      input_size=$(stat -f%z "$tmpdir/bed_chunk_$i" 2>/dev/null || stat -c%s "$tmpdir/bed_chunk_$i" 2>/dev/null || echo 0)
      
      lifted_size=0
      unmapped_size=0
      
      if [[ -f "$tmpdir/lifted_chunk_$i.bed" ]]; then
        lifted_size=$(stat -f%z "$tmpdir/lifted_chunk_$i.bed" 2>/dev/null || stat -c%s "$tmpdir/lifted_chunk_$i.bed" 2>/dev/null || echo 0)
      fi
      
      if [[ -f "$tmpdir/unmapped_chunk_$i.bed" ]]; then
        unmapped_size=$(stat -f%z "$tmpdir/unmapped_chunk_$i.bed" 2>/dev/null || stat -c%s "$tmpdir/unmapped_chunk_$i.bed" 2>/dev/null || echo 0)
      fi
      
      chunk_processed=$((lifted_size + unmapped_size))
      total_processed=$((total_processed + chunk_processed))
      
      # Consider chunk complete if output >= 95% of input (allowing for formatting differences)
      if [[ $input_size -gt 0 && $chunk_processed -ge $((input_size * 95 / 100)) ]]; then
        ((completed_chunks++))
      fi
    done
    
    # Check if all chunks are complete
    if [[ $completed_chunks -eq $ncpu ]]; then
      break
    fi
    
    # Calculate progress percentage
    if [[ $total_input_size -gt 0 ]]; then
      progress=$((total_processed * 100 / total_input_size))
    else
      progress=0
    fi
    
    elapsed=$(($(date +%s) - liftover_start))
    
    # Calculate throughput and ETA
    if [[ $total_processed -gt $last_processed && $elapsed -gt 0 ]]; then
      # Estimate ETA based on current progress rate
      remaining=$((total_input_size - total_processed))
      if [[ $total_processed -gt 0 ]]; then
        eta=$((remaining * elapsed / total_processed))
        eta_min=$((eta / 60))
        eta_sec=$((eta % 60))
        
        # Convert sizes to human readable
        processed_mb=$((total_processed / 1048576))
        total_mb=$((total_input_size / 1048576))
        
        echo "   ⏳ Progress: ${progress}% (${processed_mb}MB/${total_mb}MB) | Chunks: $completed_chunks/$ncpu | Elapsed: ${elapsed}s | ETA: ${eta_min}m ${eta_sec}s"
      else
        processed_mb=$((total_processed / 1048576))
        total_mb=$((total_input_size / 1048576))
        echo "   ⏳ Progress: ${progress}% (${processed_mb}MB/${total_mb}MB) | Chunks: $completed_chunks/$ncpu | Elapsed: ${elapsed}s"
      fi
    fi
    
    last_processed=$total_processed
  done
) &
monitor_pid=$!

# Track PIDs for error checking
pids=()
for i in $(seq -f "%02g" 1 $ncpu); do
  (
    if liftOver "$tmpdir/bed_chunk_$i" "$chain_file" \
                "$tmpdir/lifted_chunk_$i.bed" \
                "$tmpdir/unmapped_chunk_$i.bed" 2>&1 | tee "$tmpdir/liftover_$i.log" | grep -q "ERROR"
    then
      echo "❌ ERROR in chunk $i" >&2
      exit 1
    fi
  ) &
  pids+=($!)
done

# Wait for all jobs and check for failures
failed=0
for pid in "${pids[@]}"; do
  if ! wait "$pid"; then
    failed=1
  fi
done

# Stop progress monitor
kill $monitor_pid 2>/dev/null
wait $monitor_pid 2>/dev/null

if [[ $failed -eq 1 ]]; then
  echo "❌ ERROR: One or more liftOver jobs failed. Check logs in $tmpdir" >&2
  exit 1
fi

liftover_end=$(date +%s)
liftover_duration=$((liftover_end - liftover_start))
echo "✅ All liftOver jobs completed in ${liftover_duration}s"

# === Merge lifted/unmapped outputs ===
echo "🔀 Merging lifted chunks..."
cat "$tmpdir"/lifted_chunk_*.bed > "$lifted_all"
cat "$tmpdir"/unmapped_chunk_*.bed > "$unmapped_bed" 2>/dev/null || touch "$unmapped_bed"

lifted_count=$(wc -l < "$lifted_all" | tr -d '[:space:]')
unmapped_count=$(wc -l < "$unmapped_bed" | tr -d '[:space:]')
echo "   Lifted: $lifted_count fragments ($(awk "BEGIN {printf \"%.1f\", ($lifted_count/$line_count)*100}")%)"
echo "   Unmapped: $unmapped_count fragments ($(awk "BEGIN {printf \"%.1f\", ($unmapped_count/$line_count)*100}")%)"

# === Join lifted BED with barcode/count - OPTIMIZED ===
echo "🔗 Joining lifted coordinates with fragment data..."
# Use paste directly - much faster than awk for line-by-line matching
# The split command ensures chunks maintain line correspondence
paste "$lifted_all" <(
  # Extract corresponding lines from original fragments based on lifted lines
  awk 'NR==FNR{lifted[NR]=1; next} FNR in lifted {print $4"\t"$5}' \
      <(cat "$tmpdir"/lifted_chunk_*.bed) \
      <(cat "$tmpdir"/frag_chunk_*)
) | gzip -c > "$output_file"

echo "🎉 Saved lifted fragments to: $output_file"

# === Save unmapped - OPTIMIZED ===
if [[ $unmapped_count -gt 0 ]]; then
  echo "📦 Saving unmapped fragments..."
  # Extract corresponding lines from original fragments for unmapped
  awk 'NR==FNR{unmapped[NR]=1; next} FNR in unmapped {print}' \
      <(cat "$tmpdir"/unmapped_chunk_*.bed) \
      <(cat "$tmpdir"/frag_chunk_*) | gzip -c > "$unmapped_output"
  echo "   Saved to: $unmapped_output"
else
  echo "ℹ️  No unmapped fragments."
fi

# === Summary ===
end_time=$(date +%s)
total_duration=$((end_time - start_time))
echo ""
echo "================================"
echo "✅ LIFTOVER COMPLETED"
echo "================================"
echo "Total time: ${total_duration}s ($(awk "BEGIN {printf \"%.1f\", $total_duration/60}")m)"
echo "Processed: $line_count fragments"
echo "Success rate: $(awk "BEGIN {printf \"%.2f\", ($lifted_count/$line_count)*100}")%"
echo "Output: $output_file"
echo "================================"