...
#   --filter-chroms     Keep only main chromosomes (chr1-22,X,Y,2A,2B) (default: true)
...
frags_per_cell=""
seed=42
FILTER_CHROMS=true  # Keep only main chromosomes by default
...
if [[ "$FILTER_CHROMS" == true ]]; then
  echo "   Chromosome filter: ON (main chromosomes only)"
else
  echo "   Chromosome filter: OFF (all chromosomes)"
fi
...
if [[ -n "$min_frags" ]]; then
  # First count all cells (with optional chromosome filtering)
  if [[ "$FILTER_CHROMS" == true ]]; then
    all_cells_count=$($cat_cmd "$input_file" | awk '$1 ~ /^(chr)?([0-9]+|[XY]|2[AB])$/ {print $4}' | sort -u | wc -l)
  else
    all_cells_count=$($cat_cmd "$input_file" | awk '{print $4}' | sort -u | wc -l)
  fi
  
  # Then filter by minimum fragments (with optional chromosome filtering)
  if [[ "$FILTER_CHROMS" == true ]]; then
    $cat_cmd "$input_file" | awk -v min="$min_frags" '
      $1 ~ /^(chr)?([0-9]+|[XY]|2[AB])$/ { count[$4]++ }
      END {
        for (cell in count) {
          if (count[cell] >= min) {
            print cell
  ...
  if [[ "$FILTER_CHROMS" == true ]]; then
    $cat_cmd "$input_file" | awk '$1 ~ /^(chr)?([0-9]+|[XY]|2[AB])$/ {print $4}' | sort -u > "$tmpdir/all_cells.txt"
    total_cells=$(wc -l < "$tmpdir/all_cells.txt")
    total_frags=$($cat_cmd "$input_file" | awk '$1 ~ /^(chr)?([0-9]+|[XY]|2[AB])$/' | wc -l)
  else
...
if [[ -n "$ncells" ]]; then
  ...
  # Filter fragments to only selected cells (with optional chromosome filtering)
  if [[ "$FILTER_CHROMS" == true ]]; then
    $cat_cmd "$input_file" | awk 'NR==FNR{cells[$1]=1; next} $1 ~ /^(chr)?([0-9]+|[XY]|2[AB])$/ && $4 in cells' \
